...
<div class="card border-primary mb-3">
    <div class="card-header">
      ...
      <div class="card-body">
        {!! Form::open(['route' => 'actividad.actualizar', 'method' => 'POST','id' => 'FormEditActividad', 'files' => true]) !!}
        {!! Form::hidden('id', $actividades->id , ['id' => 'id']) !!}
            <div class="form-group row">
                <label for="inputEmail3" class="col-sm-2 col-form-label">Nombre Actividad</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" name="nombre" id="nombre" placeholder="Ingrese Nombre de Actividad" value="{{ $actividades->nombre }}">
                </div>
            </div>
            <div class="form-group row">
                <label for="inputPassword3" class="col-sm-2 col-form-label">Fecha Actividad</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" name="fecha" id="fecha" placeholder="Ingrese Fecha de Actividad" value="{{ $actividades->fecha }}">
                </div>
            </div>
            <div class="form-group row">
                <label for="inputPassword3" class="col-sm-2 col-form-label">Hora Inicio</label>
                <div class="col-sm-10">
                    <input type="time" class="form-control" name="hora_inicio" id="hora_inicio" min="09:00" max="18:00" value="{{ $actividades->hora_inicio }}">
                </div>
            </div>
            <div class="form-group row">
                <label for="inputPassword3" class="col-sm-2 col-form-label">Hora Término</label>
                <div class="col-sm-10">
                    <input type="time" class="form-control" name="hora_fin" id="hora_fin" min="09:00" max="18:00" value="{{ $actividades->hora_termino }}">
                </div>
            </div>
            <div class="form-group row">
                <label for="inputPassword3" class="col-sm-2 col-form-label">Detalle Actividad</label>
                <div class="col-sm-10">
                    <textarea class="form-control" name="detalles" id="detalles" rows="6">{{ $actividades->detalle_actividad }}</textarea>
                </div>
            </div>
            <div class="form-group row">
                <div class="col-sm-12">
                    <button type="button" class="btn btn-primary btn-sm btn-block" id="EditActividad">Editar Actividad</button>
                </div>
            </div>
            {!! Form::close() !!}
      </div>
           ...
           $.ajax({
	          ...
	          success: function(respuesta) {
	             if (respuesta.message == 'success') {
	                swal("OK!", "Actividad Editada Correctamente!", "success")
	                .then((value) => {
                        $("#FormEditActividad")[0].reset();
	                  location.reload();
	                });
              }
	         }
